...
def create_app():
    ...
    def create_gas_station():
        try:
            ...
            cur.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                station_id, data['name'], data.get('brand'), data.get('address'),
                float(data['latitude']), float(data['longitude']),
                data.get('gasoline_price'), data.get('ethanol_price'),
                data.get('diesel_price'), data.get('diesel_s10_price'), data.get('gnv_price')
            ))
            
            conn.commit()
            
            cur.execute("SELECT * FROM gas_stations WHERE id = ?", (station_id,))
    ...
    @app.route('/api/gas-stations/nearby', methods=['GET'])
    def get_nearby_stations():
        try:
            ...
            fuel_column = f"{fuel_type}_price"
            cur.execute(f"SELECT * FROM gas_stations WHERE {fuel_column} IS NOT NULL")
            stations = cur.fetchall()
            ...
            for station in stations:
                ...
                if distance <= radius_km:
                    ...
                    station_dict['fuel_price'] = station[fuel_column]
    ...
    def root():
        return jsonify({
            ...
            'endpoints': {
                ...
                'nearby': '/api/gas-stations/nearby'
            }
        })
